...
const Login = () => {
  ...
  const validate = () => {
    ...
    else if (!/\S+@\S+\.\S+/.test(formData.email)) newErrors.email = t('errors.invalid_email', 'Enter a valid email');
    if (!formData.password) newErrors.password = t('errors.password_required', 'Password is required');
    return newErrors;
  };
        ...
        <div className="w-full max-w-md">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-[#D2D5D9] p-8">
            <div className="mb-6 text-center sm:text-left">
              ...
              <p className="text-[#516876]">{t('auth.login_subtitle', 'Sign in to your account to continue')}</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label className="block text-sm font-medium text-[#2B373F]">{t('auth.password')}</label>
                  <Link
                    to="/forgot-password"
                    className="text-sm text-[#6794D1] hover:underline"
                  >
