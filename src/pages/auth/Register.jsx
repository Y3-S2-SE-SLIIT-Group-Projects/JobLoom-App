...
const Register = () => {
  ...
  const validateStep1 = () => {
    const errs = {};
    if (!formData.firstName.trim()) errs.firstName = t('errors.first_name_required', 'First name is required');
    if (!formData.lastName.trim()) errs.lastName = t('errors.last_name_required', 'Last name is required');
    if (!formData.email.trim()) errs.email = t('errors.email_required', 'Email is required');
    else if (!/\S+@\S+\.\S+/.test(formData.email)) errs.email = t('errors.invalid_email', 'Enter a valid email');
    ...
    else if (formData.password.length < 6) errs.password = t('errors.password_min_length', 'Password must be at least 6 characters');
    if (!formData.confirmPassword) errs.confirmPassword = t('errors.confirm_password_required', 'Please confirm your password');
    else if (formData.password !== formData.confirmPassword) errs.confirmPassword = t('errors.passwords_dont_match', 'Passwords do not match');
  ...
  const validateStep2 = () => {
    const errs = {};
    if (!formData.location.village.trim()) errs['location.village'] = t('errors.village_required', 'Village/Town is required');
    if (!formData.location.district.trim()) errs['location.district'] = t('errors.district_required', 'District is required');
    if (!formData.location.province.trim()) errs['location.province'] = t('errors.province_required', 'Province is required');
    return errs;
  };
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      setApiError(err.message || t('errors.registration_failed', 'Registration failed. Please try again.'));
    }
  };
        ...
        <div className="w-full max-w-lg">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-[#D2D5D9] p-8">
            ...
            <div className="flex items-center gap-3 mb-6">
              <div className={`flex items-center justify-center w-8 h-8 rounded-full text-sm font-bold ${step >= 1 ? 'bg-[#6794D1] text-white' : 'bg-[#F4F6F9] text-[#516876]'}`}>1</div>
              <div className={`flex-1 h-1 rounded-full ${step >= 2 ? 'bg-[#6794D1]' : 'bg-[#D2D5D9]'}`} />
              <div className={`flex items-center justify-center w-8 h-8 rounded-full text-sm font-bold ${step >= 2 ? 'bg-[#6794D1] text-white' : 'bg-[#F4F6F9] text-[#516876]'}`}>2</div>
            </div>
            ...
            {step === 1 && (
              <div className="space-y-5">
                {/* Role Selection */}
                <div>
                  <label className="block text-sm font-medium text-[#2B373F] mb-2">{t('auth.role_selection_label')}</label>
                  <div className="grid grid-cols-2 gap-3">
                    {ROLES.map(role => (
                      ...
                      >
                        <FaBriefcase className={`w-5 h-5 mb-2 ${formData.role === role.value ? 'text-[#6794D1]' : 'text-gray-400'}`} />
                ...
                <div className="flex gap-4">
                  <div className="flex-1">
                    <label className="block text-sm font-medium text-[#2B373F] mb-1.5">{t('auth.first_name')}</label>
                  ...
                  <div className="flex-1">
                    <label className="block text-sm font-medium text-[#2B373F] mb-1.5">{t('auth.last_name')}</label>
                    <input
                      type="text"
                      name="lastName"
                ...
                {formData.role === 'employer' && (
                  <div className="animate-in fade-in slide-in-from-top-2 duration-300">
                    <label className="block text-sm font-medium text-[#2B373F] mb-1.5">{t('auth.company_name')}</label>
                    <div className="relative">
                ...
                {/* Email */}
                <div>
                  <label className="block text-sm font-medium text-[#2B373F] mb-1.5">{t('auth.email')}</label>
                  <div className="relative">
                ...
                {/* Password */}
                <div>
                  <label className="block text-sm font-medium text-[#2B373F] mb-1.5">{t('auth.password')}</label>
                  <div className="relative">
            ...
            {step === 2 && (
              <form onSubmit={handleSubmit} className="space-y-5">
                <div>
                  <label className="block text-sm font-medium text-[#2B373F] mb-1.5">{t('auth.village_town')}</label>
                  <div className="relative">
                ...
                <div>
                  <label className="block text-sm font-medium text-[#2B373F] mb-1.5">{t('auth.district')}</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-sm font-medium text-[#2B373F] mb-1.5">{t('auth.province')}</label>
                  <select
